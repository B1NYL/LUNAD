...
async def t2s(text):
    ...
    async with client.audio.speech.with_streaming_response.create(
        ...
        out_path = mike_path / f"{time.time_ns()}.wav"
        await response.stream_to_file(out_path)
        return out_path

async def translate(text, domain_lang, codomain_lang):
    client = get_client()
...
async def send(token: str, text: str) -> None:
    bot = Bot(token)
    async with bot:
...
async def receive(token: str, offset=None, timeout=20):
    bot = Bot(token)
    async with bot:
        ...
        try:
            ...
            for u in updates:
                if u.message and u.message.text:
                    print(f"Received msg: {u.message.text}")
