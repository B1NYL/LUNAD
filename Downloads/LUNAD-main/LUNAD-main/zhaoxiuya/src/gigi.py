...
async def check_fall(sender_token, threshold=80):
    ...
    while True:
        ...
        if threshold < tmp:
            if not has_fall:
                printf(f"낙상이 발생하였읍니다. {pang}");
                speaker = GIGI.speakers[0]
                await sob.send(sender_token, f"낙상발생")
                printf(speaker.preset_musics()) 
                for _ in range(20):
                    speaker.play_music("Siren", 100)
                ...
                for u in updates:
                    if u.message and u.message.text:
                        ...
                        try:
                          ...
                          if data_json.get("id") != str(2):
                              printf(f"메세지 id 불일치: {data_json.get('id')} != {2}")
                              continue
